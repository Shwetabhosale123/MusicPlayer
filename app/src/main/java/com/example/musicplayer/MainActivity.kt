...
class MainActivity : AppCompatActivity() {
    ...
    //fun to access all audio files from storage
    @SuppressLint("Recycle", "Range")
    @RequiresApi(Build.VERSION_CODES.R)
    private fun getAllAudio():ArrayList<Music>{
        val tempList=ArrayList<Music>() //to access audio from storage we help cursor

        val selection=MediaStore.Audio.Media.IS_MUSIC+"!=0" //to known cursor which type of file is required
        val projection= arrayOf(MediaStore.Audio.Media._ID,MediaStore.Audio.Media.TITLE, MediaStore.Audio.Media.ALBUM,
            MediaStore.Audio.Media.ARTIST,MediaStore.Audio.Media.DURATION,MediaStore.Audio.Media.DATE_ADDED,
            MediaStore.Audio.Media.DATA,)//what data required from music file
